...
import { $createMentionNode } from "@/containers/RichEditor/plugins/MentionPlugin/MentionNode";
import { getApi } from "@/api/hooks";
import { UserDTO } from "@/api/back";
import { UserPreview } from "@/components";
import cx from "clsx";
import c from "./Mention.module.scss";
...
const dummyLookupService = {
  search(string: string, callback: (results: UserDTO[]) => void): void {
    getApi()
      .playerApi.playerControllerSearch(string, SUGGESTION_LIST_LENGTH_LIMIT)
      .then((results) => {
        callback(results);
      });
  },
...
function MentionsTypeaheadMenuItem({
  ...
  return (
    <li
      ...
      onClick={onClick}
    >
      <UserPreview nolink avatarSize={30} user={option.user} />
    </li>
  );
}
...
export default function NewMentionsPlugin(): JSX.Element | null {
  ...
  const onSelectOption = useCallback(
    (
      ...
      editor.update(() => {
        const mentionNode = $createMentionNode(selectedOption.user.steamId);
        if (nodeToReplace) {
