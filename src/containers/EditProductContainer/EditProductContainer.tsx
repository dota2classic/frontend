...
import c from "./EditProductContainer.module.scss";

interface IEditProductContainerProps {
  product?: ProductDto;
}

export const EditProductContainer: React.FC<IEditProductContainerProps> =
  observer(({ product: initialProduct }) => {
    ...
    >(() => ({
      title: initialProduct?.title || "Продукт",
      image: initialProduct?.image,
      price: initialProduct?.price || 100,
      category: initialProduct?.categoryId || categories[0]?.category,
    }));
    ...
    const save = useCallback(async () => {
      ...
      if (initialProduct?.id) {
        await getApi()
          .adminStore.adminStoreControllerUpdateProduct(initialProduct.id, {
            ...updateDto,
            category,
            imageKey: image.key,
          })
