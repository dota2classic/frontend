...
import {
  ...
  BanHammerDtoToJSON,
  CrimeLogPageDto,
  CrimeLogPageDtoFromJSON,
  CrimeLogPageDtoToJSON,
  EventAdminDto,
...
export interface AdminUserControllerCrimesRequest {
  page: number;
  perPage?: number;
  steamId?: string;
}
...
export class AdminApi extends runtime.BaseAPI {
    ...
    /**
     */
    private async adminUserControllerCrimesRaw(requestParameters: AdminUserControllerCrimesRequest): Promise<runtime.ApiResponse<CrimeLogPageDto>> {
        this.adminUserControllerCrimesValidation(requestParameters);
        const context = this.adminUserControllerCrimesContext(requestParameters);
        const response = await this.request(context);

        return new runtime.JSONApiResponse(response, (jsonValue) => CrimeLogPageDtoFromJSON(jsonValue));
    }
    ...
    /**
     */
    private adminUserControllerCrimesValidation(requestParameters: AdminUserControllerCrimesRequest) {
        if (requestParameters.page === null || requestParameters.page === undefined) {
            throw new runtime.RequiredError("page","Required parameter requestParameters.page was null or undefined when calling adminUserControllerCrimes.");
        }
    }

    /**
     */
    adminUserControllerCrimesContext(requestParameters: AdminUserControllerCrimesRequest): runtime.RequestOpts {
        const queryParameters: any = {};
    ...
    /**
     */
    adminUserControllerCrimes = async (page: number, perPage?: number, steamId?: string): Promise<CrimeLogPageDto> => {
        const response = await this.adminUserControllerCrimesRaw({ page: page, perPage: perPage, steamId: steamId });
        return await response.value();
    }

    useAdminUserControllerCrimes(page: number, perPage?: number, steamId?: string, config?: SWRConfiguration<CrimeLogPageDto, Error>) {
        let valid = true
        ...
        const context = this.adminUserControllerCrimesContext({ page: page!, perPage: perPage!, steamId: steamId! });
        return useSWR(context, valid ? () => this.adminUserControllerCrimes(page!, perPage!, steamId!) : null, config)
    }
