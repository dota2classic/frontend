...
import { RichParticipant } from "@/containers/BracketRenderer/types";
...
export class BracketsViewer {
  ...
  public async render(
    ...
    this.config = {
      ...
      rankingFormula:
        ...
        ((item): number => 3 * item.wins + 1 * item.draws + 0 * item.losses),
    };
  ...
  private renderDoubleElimination(
    ...
    if (hasLoserBracket) {
      const loserBracketMatches = splitBy(matchesByGroup[1], "round_id").map(
        (matches) => sortBy(matches, "number"),
      );
      this.renderBracket(
        container,
  ...
  private createMatch(
    ...
    if (isMatch(match)) {
      ...
      this.renderMatchDate(opponents, match);
      opponents.append(participant1, participant2);
    } else {
      const participant1 = this.createParticipant(
        match.opponent1,
  ...
  private renderParticipant(
    ...
    if (found) {
      ...
      this.renderParticipantImage(containers.name, found);
    } else
  ...
  private renderParticipantImage(
    ...
    dom.addParticipantImage(
      nameContainer,
      participant?.image_url || "/avatar.png",
    );
  }

  private renderMatchDate(
    opponents: HTMLElement,
    match: MatchWithMetadata | MatchGameWithMetadata,
  ): void {
    if (match.status >= Status.Completed) return;
    if ("games" in match) {
      const timeToDisplay = match.games
        .sort((a, b) => a.number - b.number)
        .findIndex((t) => t.status < Status.Completed);
      if (timeToDisplay !== -1) {
        const mg = match.games[timeToDisplay].scheduledDate;
        if (mg) {
          opponents.append(dom.createMatchDate(mg));
          return;
        }
      }
    }
    if ("startDate" in match) {
      const d: Date = match.startDate;
      opponents.append(dom.createMatchDate(d));
    }
  }
  ...
  private renderMatchLabel(
    ...
    if (this.config.separatedChildCountLabel) {
      opponents.append(
        dom.createMatchLabel(label, lang.getMatchStatus(match.status), onClick),
      );
